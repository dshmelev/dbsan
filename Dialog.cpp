...
void Dialog::on_buttonBox_accepted()
{
    ...
    {
        QString model_query="";
        if (!m_ui->ed_name->text().isEmpty()) model_query= model_query + "name LIKE '%" + m_ui->ed_name->text() + "%';";
        if (!m_ui->ed_user->text().isEmpty()) model_query= model_query + "user LIKE '%" + m_ui->ed_user->text() + "%';";
        if (!m_ui->ed_organization->text().isEmpty()) model_query= model_query + "organization LIKE '%" + m_ui->ed_organization->text() + "%';";
        if (!m_ui->ed_site->text().isEmpty()) model_query= model_query + "site LIKE '%" + m_ui->ed_site->text() + "%';";
        if (!m_ui->ed_mail->text().isEmpty()) model_query= model_query + "mail LIKE '%" + m_ui->ed_mail->text() + "%';";
        if (!m_ui->ed_post->text().isEmpty()) model_query= model_query + "post LIKE '%" + m_ui->ed_post->text() + "%';";
        if (!m_ui->ed_phone->text().isEmpty()) model_query= model_query + "telephone LIKE '%" + m_ui->ed_phone->text() + "%';";
        if (!m_ui->ed_data_call->text().isEmpty()) model_query= model_query + "data LIKE '%" + m_ui->ed_data_call->text() + "%';";
        if (!m_ui->ed_data_next_call->text().isEmpty()) model_query= model_query + "data_call LIKE '%" + m_ui->ed_data_next_call->text() + "%';";
        if (!m_ui->ed_internal->text().isEmpty()) model_query= model_query + "internal LIKE '%" + m_ui->ed_internal->text() + "%';";
        if (!m_ui->ed_manager_name->text().isEmpty()) model_query= model_query + "manager_name LIKE '%" + m_ui->ed_manager_name->text() + "%';";
        if (!m_ui->ed_comment->text().isEmpty()) model_query= model_query + "comment LIKE '%" + m_ui->ed_comment->text() + "%'";
